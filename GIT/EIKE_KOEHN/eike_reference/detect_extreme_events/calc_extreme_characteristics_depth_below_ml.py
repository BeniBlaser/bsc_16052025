...
scriptdir = '/home/koehne/Documents/publications/paper2/scripts_clean_initial_submission/detect_extreme_events/'   # /home/koehne/... # Euler using python/3.7.1 (gcc/6.3.0)
scriptname = 'calc_extreme_characteristics_depth_below_ml.py'
...
def load_mld_stdev(params):
    print('Load the MLD variability.')
    dir_regridded_model_output = params.dir_thresholds_and_climatologies
    varoo = 'mld_holte'
    filename = 'hobday2016_threshold_and_climatology_{}_{}zlevs_{}_{}x{}{}_downsampling_{}-{}analysisperiod_{}perc_{}-{}baseperiod_{}baseline_{}aggregation_{}smoothing_with_stdev.nc'.format(varoo,params.grid_nzlevs,params.grid_zlevels_type,params.grid_fac_horizontal_downsampling,params.grid_fac_horizontal_downsampling,params.grid_type_horizontal_downsampling,params.analysis_start_year,params.analysis_end_year,params.threshold_percentile,params.threshold_period_start_year,params.threshold_period_end_year,params.threshold_baseline,params.threshold_aggregation_window_size,params.threshold_smoothing_window_size) 
    fn = xr.open_dataset(dir_regridded_model_output+filename)
    print('Load the fields:')
    clim_365 = fn.clim_smoothed.values
    stdev_365 = fn.stdev_smoothed.values
    return clim_365, stdev_365
...
def calculate_366_arrays(clim_365, stdev_365):
    print('calculate 366 arrays.')
    clim_366 = include_feb29(clim_365)
    stdev_366 = include_feb29(stdev_365)
    return clim_366, stdev_366

def generate_and_concatenate_annual_mld_stdev_files(params):
    print('generate and concatenate annual files')
    clim_365, stdev_365 = load_mld_stdev(params)
    clim_366, stdev_366 = calculate_366_arrays(clim_365, stdev_365)
    # loop over regridded model output and compare with threshold to generate boolean
    # START THE LOOP:
    years = np.arange(1979,2020)
    leapyears = np.mod(years,4)==0
    for ydx,year in enumerate(years):
        if leapyears[ydx]==False:
            mld_stdev = stdev_365
        elif leapyears[ydx]==True:
            mld_stdev = stdev_366
        # Now concatenate all arrays
        if ydx == 0:
           mld_stdev_all = mld_stdev[:]
        else:
           mld_stdev_all = np.concatenate((mld_stdev_all,mld_stdev),axis=0)
    return mld_stdev_all

# CALCULATE THE DEPTH STATISTICS
def calc_statistics(vector):
    vec_min = np.min(vector)
    vec_max = np.max(vector)
    vec_mean = np.mean(vector)
    vec_median = np.median(vector)
    return vec_min, vec_max, vec_mean, vec_median

def calculate_depth_vectors(extreme_indices_dict):
    ...
    depth_rel_to_surface = z[unique_depths]   # ATTENTION: z has to be negatively defined
    # CALCULATE THE DEPTH RELATIVE TO THE MLD
    mld_during_event = mld[unique_times,etaidx,xiidx]      # ATTENTION: mld has to be negatively defined
    depth_rel_to_mld = depth_rel_to_surface-mld_during_event
    # CALCULATE THE DEPTH RELATIVE TO THE MLD PLUS STD
    mldstd_during_event = mld_stdev_all[unique_times,etaidx,xiidx]
    mld_plus_std_during_event = mld_during_event - mldstd_during_event # minus because mldstd is defined positively
    depth_rel_to_mld_plus_std = depth_rel_to_surface-mld_plus_std_during_event    # include the MLD std
    # return results
    return depth_rel_to_surface, depth_rel_to_mld, depth_rel_to_mld_plus_std
    
def get_chars_core(exdict,key):
    print('Calculate the extreme depth characteristics.')
    dummylen = np.zeros(len(exdict.keys()))
    
    # DEPTH REL TO SURFACE
    #depth_rel_to_surface_timeseries = []
    depth_rel_to_surface_min = np.zeros_like(dummylen)
    depth_rel_to_surface_max = np.zeros_like(dummylen)
    depth_rel_to_surface_mean = np.zeros_like(dummylen)
    depth_rel_to_surface_median = np.zeros_like(dummylen)

    # DEPTH REL TO MLD
    #depth_rel_to_mld_timeseries = []
    depth_rel_to_mld_min = np.zeros_like(dummylen)
    depth_rel_to_mld_max = np.zeros_like(dummylen)
    depth_rel_to_mld_mean = np.zeros_like(dummylen)
    depth_rel_to_mld_median = np.zeros_like(dummylen)

    # DEPTH REL TO MLD PLUS STD
    #depth_rel_to_mld_plus_std_timeseries = []
    depth_rel_to_mld_plus_std_min = np.zeros_like(dummylen)
    depth_rel_to_mld_plus_std_max = np.zeros_like(dummylen)
    depth_rel_to_mld_plus_std_mean = np.zeros_like(dummylen)
    depth_rel_to_mld_plus_std_median = np.zeros_like(dummylen)

    ###
    for eidx,ev in enumerate(exdict.keys()):
        if np.mod(eidx,10000)==0:
            print(eidx,ev)
        # CALCULATE THE TIMESERIES OF LOWER MHW BOUNDARY
        if key == 'L1D':
            depth_rel_to_surface, depth_rel_to_mld, depth_rel_to_mld_plus_std = calculate_depth_vectors(exdict[ev])
        else:
            depth_rel_to_surface, depth_rel_to_mld, depth_rel_to_mld_plus_std = [0.], [0.], [0.] 

        # CALCULATE STATISTICAL MIN/MAX/MEAN/MEDIAN DEPTH OF MHW BELOW THE SURFACE
        #depth_rel_to_surface_timeseries.append(depth_rel_to_surface)
        depth_rel_to_surface_min[eidx], depth_rel_to_surface_max[eidx], depth_rel_to_surface_mean[eidx], depth_rel_to_surface_median[eidx] = calc_statistics(depth_rel_to_surface)

        # CALCULATE STATISTICAL MIN/MAX/MEAN/MEDIAN DEPTH OF MHW BELOW THE MLD
        #depth_rel_to_mld_timeseries.append(depth_rel_to_mld)
        depth_rel_to_mld_min[eidx], depth_rel_to_mld_max[eidx], depth_rel_to_mld_mean[eidx], depth_rel_to_mld_median[eidx] = calc_statistics(depth_rel_to_mld)

        # CALCULATE STATISTICAL MIN/MAX/MEAN/MEDIAN DEPTH OF MHW BELOW THE (MLD PLUS STD)
        #depth_rel_to_mld_plus_std_timeseries.append(depth_rel_to_mld_plus_std)
        depth_rel_to_mld_plus_std_min[eidx], depth_rel_to_mld_plus_std_max[eidx], depth_rel_to_mld_plus_std_mean[eidx], depth_rel_to_mld_plus_std_median[eidx] = calc_statistics(depth_rel_to_mld_plus_std)

    chars = dict()
    # DEPTH RELATIVE TO THE SURFACE
    #chars['depth_rel_to_surface_timeseries'] = depth_rel_to_surface_timeseries  
    chars['depth_rel_to_surface_min']=depth_rel_to_surface_min
    chars['depth_rel_to_surface_max']=depth_rel_to_surface_max
    chars['depth_rel_to_surface_mean']=depth_rel_to_surface_mean
    chars['depth_rel_to_surface_median']=depth_rel_to_surface_median

    # DEPTH RELATIVE TO MLD
    #chars['depth_rel_to_mld_timeseries'] = depth_rel_to_mld_timeseries  
    chars['depth_rel_to_mld_min'] = depth_rel_to_mld_min      # deepest event depth relative to MLD
    chars['depth_rel_to_mld_max'] = depth_rel_to_mld_max      # shallowest event depth relative to MLD
    chars['depth_rel_to_mld_mean'] = depth_rel_to_mld_mean
    chars['depth_rel_to_mld_median'] = depth_rel_to_mld_median

    # DEPTH RELATIVE TO MLD PLUS STD 
    #chars['depth_rel_to_mld_plus_std_timeseries'] = depth_rel_to_mld_plus_std_timeseries  
    chars['depth_rel_to_mld_plus_std_min'] = depth_rel_to_mld_plus_std_min      # deepest event depth relative to (MLD plus STD)
    chars['depth_rel_to_mld_plus_std_max'] = depth_rel_to_mld_plus_std_max      # shallowest event depth relative to (MLD plus STD)
    chars['depth_rel_to_mld_plus_std_mean'] = depth_rel_to_mld_plus_std_mean
    chars['depth_rel_to_mld_plus_std_median'] = depth_rel_to_mld_plus_std_median
    return chars
...
def get_characteristics(exdict):
    ...
    for key in exdict:
        ...
        if key == 'E0D':
            print("E0D")
            chardict[key] = get_chars_core(exdict[key],key)
        elif key == 'L1D':
            print('L1D')
            chardict[key] = get_chars_core(exdict[key],key)
    return chardict
...
def save_depth_characteristics(params,chardict_depth,casename):
    print('Save all characteristics.')
    # Save the different dictionaries
    dict_to_save = dict()
    dict_to_save['chardict_depth'] = chardict_depth
    dict_to_save['author'] = 'E. E. Koehn'
    dict_to_save['date'] = str(date.today())
    dict_to_save['scriptdir'] = scriptdir
    dict_to_save['scriptname'] = scriptname
    dict_to_save['casename'] = casename
    dict_to_save['case_description'] = 'Given by the following attributes:'
    for pattribute in dir(params):
        if not pattribute.startswith('__') and not callable(getattr(params,pattribute)):
            ...
            dict_to_save['params.'+pattribute] = eval('params.'+pattribute)
    ...
    with open(params.dir_event_characteristics+'additional_depth_characteristics_{}.pck'.format(casename), 'wb') as f:
...
casedir = '/home/koehne/Documents/publications/paper2/scripts_clean_initial_submission/modules/cases/'   # /home/koehne/...
casenames = ['caseG1.yaml'] #['caseA1.yaml','caseB2.yaml','caseC1.yaml','caseD1.yaml','caseE1.yaml','caseF1.yaml']# #['case00_reference.yaml']
...
for casename in casenames:
    ...
    mld = load_mld(params)
    mld_stdev_all = generate_and_concatenate_annual_mld_stdev_files(params)
    ...
    #%'
    chardict_depth = get_characteristics(exdict)
    #%
    save_depth_characteristics(params,chardict_depth,casename)
